...
ast::ParsedFile rewritePackagedFile(core::Context ctx, ast::ParsedFile parsedFile) {
    ...
    if (pkg.exists()) {
        ...
        auto isTestFile = core::packages::PackageDB::isTestFile(ctx, file);
        ...
        if (!isTestFile && file.originalSigil < pkgImpl.minStrictLevel()) {
            ...
            if (auto e = ctx.beginError(loc, core::errors::Packager::MinTypedLevelViolation)) {
                e.setHeader("`{}` is marked as `# typed: {}`, but it belongs to package `{}` which requires files "
                            ...
                            ctx.file.data(ctx).path(), core::strictLevelToString(file.originalSigil),
                            pkgImpl.name.toString(ctx), core::strictLevelToString(pkgImpl.minStrictLevel()));
            }
        }
        if (isTestFile && file.originalSigil < pkgImpl.minTestStrictLevel()) {
            ...
            if (auto e = ctx.beginError(loc, core::errors::Packager::MinTypedLevelViolation)) {
                e.setHeader("`{}` is marked as `# typed: {}`, but it belongs to package `{}` which requires test "
                            ...
                            ctx.file.data(ctx).path(), core::strictLevelToString(file.originalSigil),
                            pkgImpl.name.toString(ctx), core::strictLevelToString(pkgImpl.minTestStrictLevel()));
            }
        }
        ...
        parsedFile = wrapFileInPackageModule(ctx, move(parsedFile), pkgImpl.privateMangledName, pkgImpl, isTestFile);
    } else {
